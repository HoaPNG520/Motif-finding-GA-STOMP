...
static float vec_mean(const std::vector<float>& v) {
    float s = 0; for (float x : v) s += x; return s / (float)v.size();
}
...
void MultiseedReport::finalize()
{
    ...
    mean_m    = vec_mean(ms);    std_m    = vec_std(ms,    mean_m);
    mean_ez   = vec_mean(ezs);   std_ez   = vec_std(ezs,   mean_ez);
    mean_fit  = vec_mean(fits);  std_fit  = vec_std(fits,  mean_fit);
    mean_time = vec_mean(times); std_time = vec_std(times, mean_time);
}
...
void MultiseedReport::save_csv(const char* path) const
{
    ...
    if (!fp) { fprintf(stderr, "[bench] Cannot write '%s'\n", path); return; }
...
static float spearman(const std::vector<float>& x, const std::vector<float>& y)
{
    ...
    std::sort(ix.begin(), ix.end(), [&](int a, int b){ return x[a] < x[b]; });
    std::sort(iy.begin(), iy.end(), [&](int a, int b){ return y[a] < y[b]; });
    for (int i = 0; i < n; i++) { rx[ix[i]] = i; ry[iy[i]] = i; }
    ...
    int          min_k)
{
    ...
    for (auto& r : report.rows) { fits.push_back(r.fitness); recalls.push_back(r.recall); }
...
void SweepReport::print() const
{
    ...
    printf("+------+--------+--------+----------+----------+-----------+\n");
    printf("|  m   |  fit   | recall | top_dist | entropy  | contrast  |\n");
    printf("+------+--------+--------+----------+----------+-----------+\n");
    for (auto& r : rows) {
        printf("| %4d | %.4f |  %.1f   | %8.4f | %8.4f | %9.4f |\n",
               r.m, r.fitness, r.recall, r.top_dist, r.entropy, r.contrast);
    }
    printf("+----------------------------------------------------------+\n");
...
void SweepReport::save_csv(const char* path) const
{
    ...
    fprintf(fp, "m,fitness,recall,top_dist,entropy,contrast\n");
    for (auto& r : rows) {
        fprintf(fp, "%d,%.6f,%.1f,%.6f,%.6f,%.6f\n",
                r.m, r.fitness, r.recall, r.top_dist, r.entropy, r.contrast);
    }
    fclose(fp);
