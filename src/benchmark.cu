    ...
    int min_k)
{
    ...
    for (int m = m_min; m <= m_max; m += step_size)
    {
        ...
        row.autocorr_peak = fs.autocorr_peak;
...
{
    printf("\n+-----------------------------------------------------------------------------+\n");
    printf("|                      FITNESS-RECALL SWEEP RESULTS (V7)                      |\n");
    printf("+------+--------+--------+----------+----------+----------+----------+--------+\n");
    printf("|  m   |  fit   | recall | top_dist | autocorr |   reg    | consist  | contra |\n");
    printf("+------+--------+--------+----------+----------+----------+----------+--------+\n");
    for (auto &r : rows)
    {
        printf("| %4d | %.4f |  %.1f   | %8.4f | %8.4f | %8.4f | %8.4f | %.4f |\n",
               r.m, r.fitness, r.recall, r.top_dist, r.autocorr_peak, r.regularity, r.consistency, r.contrast);
    }
    printf("+-----------------------------------------------------------------------------+\n");
...
void SweepReport::save_csv(const char *path) const
{
    ...
    fprintf(fp, "m,fitness,recall,top_dist,autocorr_peak,regularity,consistency,contrast\n");
    for (auto &r : rows)
    {
        fprintf(fp, "%d,%.6f,%.1f,%.6f,%.6f,%.6f,%.6f,%.6f\n",
                r.m, r.fitness, r.recall, r.top_dist, r.autocorr_peak, r.regularity, r.consistency, r.contrast);
    }
    fclose(fp);
