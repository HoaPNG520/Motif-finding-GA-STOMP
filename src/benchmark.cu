...
void SweepReport::print() const
{
    ...
    printf("+------+--------+--------+----------+----------+-----------+\n");
    printf("|  m   |  fit   | recall | top_dist | autocorr | contrast  |\n");
    printf("+------+--------+--------+----------+----------+-----------+\n");
    for (auto &r : rows)
    {
        printf("| %4d | %.4f |  %.1f   | %8.4f | %8.4f | %9.4f |\n",
               r.m, r.fitness, r.recall, r.top_dist, r.autocorr_peak, r.contrast);
    }
    printf("+----------------------------------------------------------+\n");
...
void SweepReport::save_csv(const char *path) const
{
    ...
    fprintf(fp, "m,fitness,recall,top_dist,autocorr_peak,contrast\n");
    for (auto &r : rows)
    {
