...
//
//  Both are O(nm) in total work, dominated by the O(n²) STOMP main loop.
// =============================================================================
...
// then computes variance via E[X²] - E[X]² in a single pass.
__global__ void compute_stats_kernel(
    const float* __restrict__ T,    // [n]   time series
    float*       means,             // [L]   output: subsequence means
    float*       stds,              // [L]   output: subsequence stds (σ ≥ EPS)
    int n,                          // series length
    int m                           // window size
);

// One thread per diagonal d ∈ [ez, L).
// Computes QT_init[d - ez] = Σ_{k=0}^{m-1} T[k] * T[k+d]  (dot product)
