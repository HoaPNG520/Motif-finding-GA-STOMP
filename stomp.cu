...
//
//  Parallelism model:
//    One CUDA thread handles one diagonal d (d ∈ [ez, L)).
...
//    6. Clamp Pearson ∈ [-1,1]              → numerical safety for sqrt
// =============================================================================
    ...
    int n, int m, int ez)
{
    ...
    // QT_init[d-ez] = Σ_{k=0}^{m-1} T[k] * T[k+d]   (first element of diag d)
    ...
    for (int i = 0; i < diag_len; i++) {
        ...
        if (sig_prod < EPS) {
            ...
            // Pearson correlation: ρ = (QT/m - μ_i·μ_j) / (σ_i·σ_j)
            // ED_z = sqrt(2m(1 - ρ))
    ...
    for (int d = ez; d < L; d++) {
        ...
        for (int i = 0; i < diag_len; i++) {
            ...
            for (int k = 0; k < m; k++) {
                ...
                sum_i += vi; sum2_i += vi * vi;
                sum_j += vj; sum2_j += vj * vj;
            }
...
STOMPResult run_stomp(const float* h_T, int n, const STOMPConfig& cfg)
{
    ...
    // Initialise MP to +∞ so atomicMinFloat always updates on first write
    {
