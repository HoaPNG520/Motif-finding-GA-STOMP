...
//
//  Both are dominated by the O(n²) STOMP kernel; they add negligible overhead.
//
//  Memory notes:
...
// ─────────────────────────────────────────────────────────────────────────────
//  Uses single-pass E[X] and E[X²] to avoid a second global-memory sweep.
//  Numerical note: var = E[X²] - E[X]²  is susceptible to catastrophic
    ...
    int n, int m)
{
    ...
    // shared memory would be ~m×.  We rely on L1 / read-only cache here.
    for (int k = 0; k < m; k++) {
        float v = __ldg(&T[i + k]);
        sum  += v;
        sum2 += v * v;
...
//  O(nm) approach is simpler and dominated by the O(n²) STOMP step anyway.
