// =============================================================================
//  ga.cu -- Genetic Algorithm for [window_size x ez_factor x min_motif_count]
//
...
//    Select     : k-tournament (default k=3)
//    Crossover  : arithmetic blend for continuous genes;
//                 random-parent inheritance for discrete min_motif_count
//    Mutate     : Gaussian perturbation with temperature annealing
...
// -----------------------------------------------------------------------------
//  alpha in [0.3, 0.7] keeps the child between the two parents, preventing
//  rapid drift to bounds.
//  window_size  -> arithmetic blend, rounded to nearest int
//  ez_factor    -> arithmetic blend (continuous)
//  min_motif_k  -> random inherit from one parent (blending a count is meaningless)
    ...
    const GAConfig &cfg)
{
    ...
    float blended_m = alpha * (float)a.window_size + (1.0f - alpha) * (float)b.window_size;
    child.window_size = clampVal((int)roundf(blended_m), cfg.m_min, cfg.m_max);

    float blended_ez = alpha * a.ez_factor + (1.0f - alpha) * b.ez_factor;
...
// -----------------------------------------------------------------------------
//  temperature in (0, 1] decreases each generation so the search narrows.
//  sigma_m  = (m_max  - m_min)  * 0.10 * temperature
//  sigma_ez = (ez_max - ez_min) * 0.10 * temperature
Individual mutate(
    Individual ind,
    float temperature,
    const GAConfig &cfg)
{
    if ((float)rand() / RAND_MAX < cfg.mutation_rate)
    {
        ...
        ind.window_size = clampVal(ind.window_size + delta, cfg.m_min, cfg.m_max);
    }
    ...
    if ((float)rand() / RAND_MAX < cfg.mutation_rate)
    {
        ...
        ind.ez_factor = clampVal(ind.ez_factor + delta, cfg.ez_min, cfg.ez_max);
    }
